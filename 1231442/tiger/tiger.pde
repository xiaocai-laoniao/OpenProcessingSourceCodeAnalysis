...
final color BG_COLOR 		 = color(197,69,63); // 画布背景填充色
final color FG_COLOR     = color(240,188,96); // 粒子的前景填充色
final color BORDER_COLOR = color(110,22,18); // 粒子的边框色
...
void setup() {
  ...
  font = createFont("AlibabaPuHuiTi-Heavy-48.vlw", 350);
  textFont(font); // 设定字体
  fill(0); // 字体填充为黑色
  textAlign(CENTER, CENTER); // 设定字体的对齐模式，水平居中，垂直居中
  text(typedText, width/2, height/2); // 显示字体
  typedText = "";
  inputText = "";
  
  //tigerImg = loadImage("tiger.png");
  //tigerImg.resize(width, height);
  //image(tigerImg, 0, 0);
